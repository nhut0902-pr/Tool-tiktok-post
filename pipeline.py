...
def process_all_pending():
    ...
    for link_id, url in rows:
        try:
            ...
            c.execute(
                "UPDATE links SET status='done', result=? WHERE id=?",
                (json.dumps(result, ensure_ascii=False), link_id)
            )
        except Exception as e:
            c.execute(
                "UPDATE links SET status='error', result=? WHERE id=?",
                (str(e), link_id)
            )
    conn.commit()
    conn.close()
...
def process_scheduled():
    ...
    c = conn.cursor()
    c.execute(
        "SELECT id, url, schedule_time FROM links WHERE status='pending' "
